...
export class ChallengeParticipationService {
  ...
  async create(createChallengeParticipation: CreateChallengeParticipationDto) {
    ...
    if (user.challengeParticipation) throw new BadRequestException('Usuario ya está participando en una challenge');
    const challenge: Challenge = await this.challengesService.findOne(challengeId);
  ...
  async removeAll(): Promise<void> {
    const challengeParticipations = await this.challengeParticipationsRepository.find();
    await this.challengeParticipationsRepository.remove(challengeParticipations);
  }
}
