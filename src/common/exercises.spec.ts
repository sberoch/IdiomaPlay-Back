...
describe('Exercises json testing', () => {
  ...
  it('Every exercise to have a correct option in the options array', () => {
    for (const exercise of exercisesJson) {
      const foundOption = exercise.options.find(actualOption => actualOption === exercise.correctOption)
      expect(foundOption).toBeDefined();
    }
  });

  it('Every exercise of type "complete" should have 4 options', () => {
    const completingExercises = exercisesJson.filter(exercise => exercise.type === ExerciseType.COMPLETE)
    for (const actualCompleteExercise of completingExercises) {
      expect(actualCompleteExercise.options.length === config.amountOfOptionsPerCompletingExercise)
    }
  })

  it('Every exercise of listening and translation should have 6 options', () => {
    const notCompletingExercises = exercisesJson.filter(exercise => exercise.type !== ExerciseType.COMPLETE)
    for (const exercise of notCompletingExercises) {
      expect(exercise.options.length === config.amountOfOptionsPerListeningAndTranslatingExercise)
    }
  })
});